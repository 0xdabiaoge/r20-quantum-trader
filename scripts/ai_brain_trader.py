...
1. 多空双向对称顺势原则（Dual-Direction Trend Following）：
   ...
   - **做多条件（4H多头通道或箱体下沿）**：
     ...
     - 1H 回调、减速（BEAR_DECELERATING）定性为寻找支撑位（VWAP、EMA、颈线）的打折买点；
     - 箱体震荡下沿触及强支撑且 RSI 超卖时，果断低吸做多；
     - 此时 100% 严禁任何逆势摸顶开空。
   - **做空条件（4H空头承压或箱体上沿）**：
     - 当 4H 宏观受压（4H_MACRO_BEAR），或 1H 均线空头排列时，**专注顺势做空**；
     - 1H 向上弱反弹、冲高回落、触碰上方阻力均线受挫时，是极佳的逢高做空点；
     - 箱体震荡上沿触及强阻力且 RSI 超买受阻时，果断高抛做空；
     - 此时 100% 严禁任何逆势抄底做多。
   - **历史做空胜率低的根本原因纠偏**：
     - 过去做空胜率仅 25% 不是因为“不能做空”，而是因为在多头趋势中“过早逆势摸顶猜顶”，或者在破位时“右侧追在最低点地板上被插针”。
     - 正确的高胜率做空姿势是：**等待弱势反弹至阻力位衰竭受挫，挂限价单在上方阻力位（做空入场价高于现价 0.2%~0.5%）舒服接空**！

2. 杜绝过窄止损与噪音插针扫损（Noise-Proof Stop Loss）：
   - 无论做多还是做空，止损必须充足宽容：严格以结构性支撑/阻力外侧加上 **1.8x ~ 2.2x 1H ATR**（或现价外 1.5%~2.5% 安全垫）作为硬止损位，彻底避开 15M 噪音杂波区间；
   - 绝不允许设定过窄止损，宁可降低杠杆（2x~3x）与仓位比例，也要给足波动呼吸空间，从物理上隔绝虚假洗盘止损！

3. 浮盈保本与阶段止盈双锁死机制（Profit Lock）：
   - 持仓一旦浮盈达到 **0.8R ~ 1.0R**（或 +1.0% ROI），无论多单还是空单，必须立即主动输出 `UPDATE_SL` 将止损拉升至开仓成本位（保本价 BE）；
   - 保本位一旦建立，本单无论发生何种黑天鹅反抽，**最坏结果也是 0 亏损保本退出**，彻底将潜在亏损单转化为保本胜率保护！
   - 达到 1.5R ~ 2.0R 关键阻力/支撑位时建议部分止盈锁定真金白银。

4. 宁缺毋滥的优质点位挂单（Maker Pullback Entry）：
   - 严禁在价格中枢（箱体中间半山腰）开仓，只在四大高胜率点位出手：
     ① 顺势多头回踩均线/支撑位（现价下方 0.2%~0.6% 挂限价买单）；
     ② 顺势空头反弹承压阻力位（现价上方 0.2%~0.6% 挂限价空单）；
