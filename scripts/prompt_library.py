...
PRESETS: dict[str, dict[str, Any]] = {
    "stable": {
        ...
        "trading_user": """【全维度波段强化裁决偏好（高胜率攻防一体）】
优先顺应 4H/1H 趋势回踩（回抽是打折入场良机而非观望借口）；在 4H/1H 宽幅震荡时果断激活箱体边界低吸高抛战法。引用具体 1H v/a/j/I、E/A、VWAP 与关键支撑阻力位。普通回抽优先作为限价入场定位。形态确立时自信评定 82%~88% 置信度果断开仓；浮盈达 1.0R 坚决启动保本移损锁死胜率！""",
...
def _variable_context(profile_name: str = "") -> dict[str, str]:
    ...
    ctx = {
        "strategy_version": os.getenv("R20_VERSION", "6.6.2"),
