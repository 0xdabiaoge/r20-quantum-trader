...
def _urlencoded_json(url: str, data: dict[str, Any] | None = None, timeout: int = 60) -> dict[str, Any]:
    ...
    request = urllib.request.Request(url, data=body, headers={"User-Agent": "R20-Backup/6.1.0-preview"}, method="POST" if body is not None else "GET")
...
def _multipart_upload(url: str, field_name: str, filename: str, content: bytes, timeout: int = 180) -> dict[str, Any]:
    ...
    request = urllib.request.Request(url, data=body, headers={"Content-Type": f"multipart/form-data; boundary={boundary}", "User-Agent": "R20-Backup/6.1.0-preview"}, method="POST")
