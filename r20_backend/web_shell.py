...
`dashboard/app.py` 原先**自己** `FastAPI()` 建了一个应用、自己挂静态目录、自己注册
...
阶段 2·B2 收尾）把剩下的**外壳本身**也搬出来，让 `dashboard/app.py` 彻底降为纯库
（只提供 `get_all_data` / `get_overview` / `update_cache_cycle` 等实现，不再持有 app）。

## 边界

- 本模块只依赖 FastAPI/Starlette 与标准库，不 import `dashboard.app`，
  也不被 `dashboard.app` import（避免又把外壳绑回库文件）。
...
__all__ = [
    "WORKSPACE_DIR", "DASHBOARD_DIR", "VUE_DIST_DIR", "VUE_ASSETS_DIR",
