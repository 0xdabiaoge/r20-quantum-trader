...
`dashboard/app.py` 里那个 1021 行的 `update_cache_cycle` 要按数据域拆出来，
这里先承载最基础、最无副作用的一层 —— 本地文件读取。

语义约定（与拆分前的内联写法**逐条等价**，这是本次重构的安全底线）：
- 文件不存在 / 读不动 / JSON 解析失败 → 一律回退到 `default`，绝不抛错；
  `dashboard/app.py` 原来的写法是 `if os.path.exists(...): try: ... except: pass`，
  这里直接 `try: open(...) except OSError` —— 结果相同，且没有 exists→open 的 TOCTOU 窗口。
- 调用方若要区分「读不到」与「读到空」，不要用这些函数（它们刻意把两者合并为 default）。

本模块只依赖标准库；**不得**反向 import `dashboard.app`（会与
routers/dashboard.py 的 `import dashboard.app` 构成环）。
"""
...
def read_text_lines(
    ...
    strip=True 时丢弃空行（对齐 dashboard/app.py 原日志读取语义：
