...
[![Release](https://img.shields.io/badge/release-v7.9.2-orange.svg?style=flat-square)](https://github.com/555cute/r20-quantum-trader/releases/tag/v7.9.2)
...
## 📸 系统全景一览 (v7.9.2 实机高清截图)

R20 量子交易系统由**前台双翼量化操盘终端**与**后台机构级控制面**共同构成：

### 1. 前台双翼量化操盘大屏（Bento 资产舱 + 因子矩阵防抖排序 + KLineChart v10 原生 K 线工作站）
![前台操盘大屏](docs/images/v792_live_dashboard.png)

### 2. 后台机构级量化控制面（核心策略、三所执行健康与物理风控全景监控）
![后台机构级控制面](docs/images/v792_admin_overview.png)

### 3. 执行层风控管理中心（17 项硬风控可视化 + 三套优质预设一键应用 · 单一事实源）
...
| :--- | :--- | :--- | :--- |
| **1. 标的池与合约规格** | 任意 OKX / 币安 / Gate 永续合约增删、面值 (`ctVal`)、报价精度 (`tickSz`)、最小张数 (`minSz`)、基础仓位 (`base_sz`)、单笔风险暴露 (`risk_per_trade_usd`) | 前后台动态更新 / 编辑 `data/instrument_pool.json` | 告别固定币种限制，自由交易任意心仪山寨币或主流币 |
...
| **7. 执行层风控管理中心** | 最高持仓数、同向敞口、单笔保证金占比、单标的保证金封顶、杠杆上限、1R 风险额、R:R 硬底线、开仓/加仓置信度门禁、日亏熔断、最长持仓时间、止损冷静期等 17 项 | 后台风控管理页集中配置或三套优质预设一键应用，`.env` 持久化，下一巡检周期热生效 | 告别写死在 py 里的黑盒阈值，风控口径提示词/执行层/管理页三位一体 |
| **8. 三所平权执行拓扑** | OKX、Binance、Gate 统一资产穿透、多所动态保证金对账与路由拓扑、原生条件保护单分发 | 统一调度拓扑中枢管理，一处风控定义多所严格继承 | 多交易所分散风控、平权并发撮合，告别单所对手盘风险 |
...
- **全屏与自适应展开**：v7.9.2 彻底解除文本域单行高度锁，支持多行舒适阅读，并屏蔽代码波浪线干扰；
...
- **策略方案导入与导出**：一键保存当前自定义方案、复制副本、查看历史版本，导入弹窗具备严格的 JSON 校验门禁与防重入机制。
...
![自进化认知实验室](docs/images/v792_self_evolution.png)

- **实盘台账自动穿透**：实时统计复盘样本数、综合胜率、利润因子（PF），精准剖析多空双杀、手续费摩擦损耗与执行偏差；
- **白盒黄金心法沉淀**：自动生成并动态维护 `AI_TRADING_MEMORY.md` 实战心法，注入主脑每一轮交易决策；
- **防污染护栏 (Evolution Shield)**：离群噪点剔除、宪法级防偏见红线；支持一键回滚至官方黄金基准，复盘弹窗内置防误触短语严格门禁；
...
![物理拦截插件管线](docs/images/v792_interceptors_failclosed.png)
...
- **在线沙箱回归测试**：支持在界面直接输入模拟报价参数，现场单步测试每道拦截插件的通过/拒绝表现；新建插件具备严格的文件名校验与表单隔离。
...
- **双轮质询互评模式 (Cross-Exam)**：第一轮独立提案 ➔ 第二轮同行交叉漏洞质询辩论 ➔ 第三轮 CIO 统筹终审查阅拍板；支持一键切换为单模型极速直出；
...
![模型网关与全局推理配置](docs/images/v792_llm_hub.png)
...
- **模型配置必填门禁**：v7.9.2 新增模型主键防呆与属性校验，保存模型动作杜绝空 ID 无效发包。
...
- **四大策略单元聚合指纹**：将「提示词模版」、「物理拦截插件」、「投委会席位配置」与「标的池风控参数」四合一，计算出唯一不可变的 SHA-256 策略版本标识（如 `v7.9.2@8b2c4e1f`）；
- **策略方案具名归档**：支持将当前调试满意的全套自定义策略一键归档为独立文件（如“稳健趋势追随版”、“激进破位突破版”），随时切换；
...
全部执行层硬风控参数从 py 源码中彻底剥离，收敛至后台「风控管理页」集中可视化配置：
...
- **单一事实源架构**：`scripts/risk_constants.py` 为唯一参数源，执行引擎、核心风控底座（R:R 物理拦截）与后台管理页三方同源，修改一处多所通用；
- **高危操作防呆确认**：基线本金修改与紧急一键全平仓均受到严格的短语输入匹配与确认门禁保护，输入未满足条件时按钮安全禁用，并同步向读屏器上报错误态。

---

### 8. 🌐 三所平权对等执行与统一资产敞口 (Three-Venue Parity)
...
- **原生条件单适配**：币安 Conditional Algo 单（`reduceOnly=True`）与 Gate 双向模式（`in_dual_mode=True`）全自动识别适配；
...
- 资金规模不足以开出合规最小仓位（无法同时满足最小下单量、止损呼吸空间与 R:R ≥ 2.0）的标的，**直接跳过并在日志标注"资金不匹配"**，绝不放大仓位硬开；
- 全部阈值可在后台「风控管理页」可视化配置，或直接改 `.env`，二者同源。

> ⚠️ 实话：20U 虽然技术上可跑，但 Taker 0.05% 双边手续费 + 资金费率相对 1.8~2.2x ATR 的止损空间占比过高，建议用模拟盘或 ≥300U 实盘验证策略。
...
1. **OKX**：在 OKX 分别创建 **DEMO / LIVE** API Key，填齐 **API Key、Secret Key、Passphrase** 三件套。仅授予必要的读取与交易权限，禁止提款权限，绑定服务器 IP；
2. **Binance (币安)**：支持 U 本位永续合约 API Key 与 Secret Key 接入，自动识别条件保护单；
3. **Gate (芝麻开门)**：支持永续合约 V4 API 接入，自动识别单向/双向持仓模式；
4. **安全加固存储**：后台所有交易所密钥保存到本地 **Fernet 工业级加密存储**；输入框留空表示不改，不会明文泄露至日志或 Git。

---

## 🧪 全栈自动化测试保障

系统配备了涵盖物理风控几何拦截、策略版本快照、多模型仲裁、三所鉴权与前后端 API 契约的完整自动化测试套件：

```bash
# 1. 前端质量与无障碍守卫测试 (389 项全过)
cd frontend && node --test tests/*.test.mjs

# 2. 端到端 Playwright UI 测试 (175 项全过)
pytest tests/ui -q

# 3. 后端运维与安全审计综合测试 (571 项全过)
pytest tests/ops tests/audit -q
```
...
3. 强烈建议先在交易所 **DEMO 模拟盘**环境下充分验证策略与风控逻辑；
